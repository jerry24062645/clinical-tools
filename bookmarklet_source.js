(()=>{
const ID='__acl648', KEY='__acl648cache';
...
function parsePVR(t){
 ...
 // Fallback for HIS reports where blank pipe columns collapse oddly.
 if(!P.R){let m=t.match(/NORMAL\s*\|\s*(\d+(?:\.\d+)?)/i);if(m){P.R=m[1];P.Rstatus='normal'}}
 if(!P.L){let m=t.match(/ABNORMAL\s*\|(?:\s*\|)?\s*(\d+(?:\.\d+)?)\s*\|/i);if(m){P.L=m[1];P.Lstatus='abnormal'}}
 save();
}
