...
function parseLabs(){
 ...
 // FIB-4 = (Age × AST) / (Platelets × sqrt(ALT)), grouped strictly by completion date.
 for(const [date,g] of Object.entries(S.byDate)){
   const L=g.labs||{}, ageN=Number(patientAgeSex().age);
   const ast=parseFloat(String(L.ast?.raw??'').replace(/^[<>≤≥]/,''));
   const alt=parseFloat(String(L.alt?.raw??'').replace(/^[<>≤≥]/,''));
   const plt=parseFloat(String(L.plt?.raw??'').replace(/^[<>≤≥]/,''));
   if(Number.isFinite(ageN)&&ageN>0&&Number.isFinite(ast)&&Number.isFinite(alt)&&alt>0&&Number.isFinite(plt)&&plt>0){
     const x=(ageN*ast)/(plt*Math.sqrt(alt));
     L.fib4={v:String(Number(x.toFixed(2))),raw:String(x),unit:'',calculated:true,caution:(ageN<35||ageN>65)};
     changed=true;
   }
 }
...
function formatGroup(g){
 ...
 if(v(L,'fib4')){let z=`• FIB-4 Score ${v(L,'fib4')}`;if(L.fib4?.caution)z+=' (Use with caution in patients <35 or >65 years old; FIB-4 is less reliable in these patients.)';lines.push(z)}
