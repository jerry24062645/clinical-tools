...
function ensureDate(date){
  if(!date)return null;
  if(!S.byDate[date])S.byDate[date]={labs:{},urine:{},bloodGas:{},cbcDiff:{},series:{poc:[]},stool:{},special:{},echo:{},inbody:{}};
  const g=S.byDate[date];
  g.labs||={};g.urine||={};g.bloodGas||={};g.cbcDiff||={};g.series||={poc:[]};g.series.poc||=[];g.stool||={};g.special||={};g.echo||={};g.inbody||={};
  return g;
}
...
function parseAll(){parseLabs();parseCBCDiff();parseUrine();parseBloodGas();parseSpecialText();parseEcho(txt());parseInbody(txt())}
function v(L,k){return L[k]?.v}
function formatGroup(g){
 const L=g.labs||{},U=g.urine||{},E=g.echo||{},I=g.inbody||{},D=g.cbcDiff||{},BG=g.bloodGas||{},SP=g.special||{},ST=g.stool||{},lines=[];let a=[];
...
d.querySelector('#aC').onclick=()=>navigator.clipboard.writeText(R.textContent);d.querySelector('#aK').onclick=()=>{S={byDate:{},mode:'AUTO'};sessionStorage.removeItem(KEY);R.textContent='';CAP.style.display='none';IMG.removeAttribute('src');setMsg('Cache cleared.');};d.querySelector('#aX').onclick=()=>d.remove();d.querySelector('#aS').onclick=async()=>{if(window.isSecureContext&&navigator.mediaDevices?.getDisplayMedia){await directCapture();return}setMsg('This HIS page cannot capture the screen directly. Opening the secure capture helper…');const helper='__CAPTURE_HELPER__';const w=open(helper,'aclScreenCapture','width=920,height=720,resizable=yes,scrollbars=yes');if(!w)setMsg('Popup blocked. Please allow pop-ups, then click Capture screen again.',true)};draw();
})()
